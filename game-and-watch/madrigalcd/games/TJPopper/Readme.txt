Product:	Tom & Jerry Popper (Gakken, LCD Card Game) Simulator
Version: 	5.00 (Standard 5, complete rewrite)
Started:	29th November, 2000
First release:	 3rd January,  2001
...
The author is in no way affiliated with Gakken Inc.
"Tom & Jerry Popper"	(C) Gakken
"LCD Card Game"		(C) Gakken
"Tom & Jerry"		(C) MGM/UA, 1983
...
* Press left  arrow key to play game A (easy). Highest score is displayed first.
* Press right arrow key to play game B (hard). Highest score is displayed first.
* Instructions:
  - Tom is flying balloon bombs!
  - Break them quickly by the saber.
  - It's Jerry's mistake when you failed to break balloons or the saber was
    taken by the magnet (in game B only).
  - The game ends when you make 3 mistakes.
* Score:
  - 1 point per broken balloon
  - 1 point per broken magnet (game B only)
* Maximum display score is 999 points..
* Highest scores are saved in the System Registry.
* After game ends, time function displays after about 90 seconds.
...
A) by MOUSE and TOUCH SCREEN
   ...
   Left/Right buttons	Start Game A/B (in time mode)
   			Move the main character left/right (in game mode)

B) by KEYBOARD

   1 (*)		Time (hold to view alarm time)
   5 (*)		Reset the game

   Arrow keys (*)       Start Game A/B (in time mode)
      			Move the main character left/right (in game mode)
